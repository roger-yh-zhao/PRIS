...
from .sparse_attention_layer import SparseAttention

from .gt_config import set_cfg_gt
...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
                 local_gnn_type, global_model_type, num_heads, act='relu', #'relu',
                 pna_degrees=None, equivstable_pe=False, dropout=0.15,
                 attn_dropout=0.15, layer_norm=False, batch_norm=True,
        ...
        self.activation = nn.ReLU()#F.relu fail for GINE
        ...
        elif local_gnn_type == 'ASGEN':
            phi_model = pygnn.GENConv(dim_h, dim_h)
            self.local_model = pygnn.AntiSymmetricConv(
                                             in_channels=dim_h,
                                             phi=phi_model,
                                             epsilon=0.1,
                                             gamma=0.1,
                                             num_iters=1)
        elif local_gnn_type == 'ASGAT':
            phi_model = pygnn.GATConv(in_channels=dim_h,
                                             out_channels=dim_h // num_heads,
                                             heads=num_heads,
                                             edge_dim=dim_h)
            self.local_model = pygnn.AntiSymmetricConv(
                                             ...
                                             gamma=0.1,
                                             num_iters=1)
        elif local_gnn_type == 'ASGINE':
            gin_nn = nn.Sequential(Linear_pyg(dim_h, dim_h),
                                   self.activation,
                                   Linear_pyg(dim_h, dim_h))
            if self.equivstable_pe:  # Use specialised GINE layer for EquivStableLapPE.
                phi_model = GINEConvESLapPE(gin_nn)
            else:
                phi_model = pygnn.GINEConv(gin_nn)
            #phi_model = pygnn.LEConv(dim_h, dim_h)
            self.local_model = pygnn.AntiSymmetricConv(
                                             in_channels=dim_h,
                                             phi=phi_model,
                                             epsilon=0.1,
        ...
        if global_model_type == 'None':
            ...
            #     dim_feedforward=2048, dropout=0.1, activation=nn.ReLU(),
            ...
            self.self_attn = SparseAttention(
                dim=dim_h, kq_dim=10, val_dim=dim_h//num_heads,
                num_heads=num_heads, k=15,
                head_agg="None",
                random_attention=False,
                random_fraction=0.1
            )
        else:
            raise ValueError(f"Unsupported global model: "
                             f"{global_model_type}")
        ...
        if self.layer_norm:
            ...
            # self.norm1_global = pygnn.norm.GraphNorm(dim_h)
            # self.norm1_local = pygnn.norm.InstanceNorm(dim_h)
            # self.norm1_global = pygnn.norm.InstanceNorm(dim_h)
        if self.batch_norm:
    ...
    def _apply_self_attn(self, h, graph_ids):
        ...
        if self.global_model_type == 'Transformer':
            h_attn = self._gl_block(h_dense, None, ~mask)[mask]
    ...
    def forward(self, batch):
        ...
        if self.has_global:
            if 0:#cfg.share.gradient_checkpointing:
                h_global = checkpoint(GPSLayer._apply_self_attn,
                    self, h, batch.batch
                )
            else:
                h_global = self._apply_self_attn(h, batch.batch)
