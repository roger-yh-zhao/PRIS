...
def run_a_train_epoch(epoch, model, data_loader, optimizer, mdn_weight=1.0, affi_weight=1.0, aux_weight=0.001, dist_threhold=None, device='cpu'):
	...
	for batch_id, batch_data in enumerate(data_loader):
		...
		nuc_labels = th.argmax(bgl.x[:,:12], dim=1, keepdim=False)#;print('see',bgl.x[:,:17])
		...
		pi, sigma, mu, dist, nuc_types, bond_types, batch = model(bgl, bgp)
		...
		loss = (mdn * mdn_weight) + (affi * affi_weight) + (nuc * aux_weight) + (bond * aux_weight)
		probs.append(y)
		...
		nuc_loss += nuc.item() * batch.unique().size(0)
		bond_loss += bond.item() * batch.unique().size(0)
		total_loss += mdn_loss + nuc_loss * aux_weight + bond_loss * aux_weight
		
		#print('Step, Total Loss: {:.3f}, MDN: {:.3f}'.format(total_loss, mdn_loss))
		if np.isinf(mdn_loss) or np.isnan(mdn_loss): break
		del bgl, bgp, nuc_labels, bond_labels, pi, sigma, mu, dist, nuc_types, bond_types, batch, mdn, nuc, bond, affi, loss, prob, y
		th.cuda.empty_cache()
	
	ys = th.cat(probs)
	affi_loss = th.corrcoef(th.stack([ys, data_loader.dataset.labels.to(device)]))[1,0].item()
		
	return total_loss / len(data_loader.dataset)+ affi_loss* affi_weight, mdn_loss / len(data_loader.dataset), affi_loss, nuc_loss / len(data_loader.dataset), bond_loss / len(data_loader.dataset)
	...
	with th.no_grad():
		for batch_id, batch_data in enumerate(data_loader):
			...
			if pred or atom_contribution or res_contribution:
				prob = calculate_probablity(pi, sigma, mu, dist)#;print(len(dist))
				if dist_threhold is not None:
				...
				if pred:
					...
					probx = scatter_add(prob, batch[th.where(dist <= dist_threhold)[0]] , dim=0, dim_size=batch.unique().size(0))
					probs.append(probx)
				...
				nuc = F.cross_entropy(nuc_types, nuc_labels)
				bond = F.cross_entropy(bond_types, bond_labels)
				loss = (mdn * mdn_weight) + (nuc * aux_weight) + (bond * aux_weight)
			...
			del bgl, bgp, nuc_labels, bond_labels, pi, sigma, mu, dist, nuc_types, bond_types, batch 
			th.cuda.empty_cache()
